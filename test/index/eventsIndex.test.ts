...
describe('EventsIndex', () => {
  ...
  it('indexes events.xml files', () => {
    expect(project.indexes.events.getFileCount()).toBeGreaterThanOrEqual(1);
  });
  ...
  it('finds multiple observers for the same event', () => {
    const observers = project.indexes.events.getObserversForEvent('test_foo_load_after');
  ...
  it('finds observer registrations by FQCN', () => {
    const refs = project.indexes.events.getObserversForFqcn(
  ...
  it('returns empty for unknown event', () => {
    expect(
      project.indexes.events.getObserversForEvent('nonexistent_event'),
    ).toHaveLength(0);
  });

  it('returns empty for unknown FQCN', () => {
    expect(
      project.indexes.events.getObserversForFqcn('Unknown\\Class'),
    ).toHaveLength(0);
  });

  it('finds event name references', () => {
    const refs = project.indexes.events.getEventNameRefs('test_foo_save_after');
    expect(refs).toHaveLength(1);
  ...
  it('finds reference at cursor position', () => {
    ...
    const found = project.indexes.events.getReferenceAtPosition(
      eventRef.file,
      eventRef.line,
      eventRef.column,
    );
